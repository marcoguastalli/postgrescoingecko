...
public class ApiController {
    ...
    public ResponseEntity<?> getLatterBlockchainBlocks(@NonNull HttpServletRequest httpServletRequest) {
        ...
        try {
            ...
            // get previous coins from the DDBB
            final List<Coin> coinsInDatabase = coinsService.findAll();

            // do
            int numberOfCoins = 0;
            final byte[] jsonInBytes = apiService.getJson(trackingId, applicationYmlConfig.getUrlToCall());
            ...
            if (null != currentCoins) {
                ...
                // Delete coins that were removed from the API
                if (!deltaResult.getRemoved().isEmpty()) {
                    logInfoTrackingId(log, trackingId, format("Deleting %s removed coins", deltaResult.getRemoved().size()));
                    coinsService.deleteAll(deltaResult.getRemoved());
                }
